...
public class DataInitializer implements CommandLineRunner {
  ...
  private void addStockSymbols() {
    try {
      List<CsvBean> csvBeans =
          beanBuilder(Paths.get("src/main/resources/stocks/amexlist.csv"), StockSymbol.class);
      csvBeans.addAll(
          beanBuilder(Paths.get("src/main/resources/stocks/nasdaqlist.csv"), StockSymbol.class));
      csvBeans.addAll(
          beanBuilder(Paths.get("src/main/resources/stocks/nyselist.csv"), StockSymbol.class));
  ...
  private void addCountries() throws java.io.IOException {
    ...
    Country[] countries =
        mapper.readValue(new File("src/main/resources/countries.json"), Country[].class);
    int i = 0;
        ...
        .forEach(
            i -> {
              ...
              taskExecutor.execute(
                  new ThreadedDataInitializer(
                      roleRepository, userRepository, startIndex, range, passwordEncoder));
            });
  }
}

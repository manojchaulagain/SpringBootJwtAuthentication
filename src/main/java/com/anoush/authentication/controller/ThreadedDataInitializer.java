...
/** Created using IntelliJ IDEA User: Manoj Chaulagain Date: 2019-04-28 Time: 12:35 */
@Slf4j
public class ThreadedDataInitializer implements Runnable {
  ...
  public ThreadedDataInitializer(
  ...
  public void run() {
    try {
      IntStream.range(startIndex, startIndex + range)
          .forEach(
              i -> {
  ...
  private void addUser(Role savedRole, String name, String userName, String email) {
    ...
    userRepository.save(
        new User(name, name, userName, email, passwordEncoder.encode("N1e2p3al!"), roleSet));
